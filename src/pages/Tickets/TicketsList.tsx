    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-ink-400">{filtered.length} ticket{filtered.length !== 1 && 's'} found</p>
        </div>
        {user?.role === 'employee' && (
      ...
      <div className="flex flex-wrap gap-2 rounded-lg border border-ink-100 bg-white p-3 shadow-card">
        <input
          value={search}
          onChange={(e) => { setSearch(e.target.value); setPage(1) }}
          placeholder="Search by ticket ID, subject, requester or agent…"
          className="min-w-[220px] flex-1 rounded-md border border-ink-200 px-3 py-2 text-sm focus:border-steel-400 focus:outline-none focus:ring-2 focus:ring-steel-100"
        />
        <select value={status} onChange={(e) => { setStatus(e.target.value); setPage(1) }} className="rounded-md border border-ink-200 px-3 py-2 text-sm">
          <option value="">All statuses</option>
          {statuses.map((s) => <option key={s} value={s}>{s}</option>)}
        </select>
        <select value={priority} onChange={(e) => { setPriority(e.target.value); setPage(1) }} className="rounded-md border border-ink-200 px-3 py-2 text-sm">
          <option value="">All priorities</option>
          {priorities.map((p) => <option key={p} value={p}>{p}</option>)}
        </select>
        <select value={category} onChange={(e) => { setCategory(e.target.value); setPage(1) }} className="rounded-md border border-ink-200 px-3 py-2 text-sm">
          <option value="">All categories</option>
          {categories.map((c) => <option key={c.id} value={c.name}>{c.name}</option>)}
        </select>
        <select value={sort} onChange={(e) => setSort(e.target.value as SortKey)} className="rounded-md border border-ink-200 px-3 py-2 text-sm">
        ...
        {(search || status || priority || category || sort !== 'newest') && (
          <button onClick={resetFilters} className="rounded-md px-3 py-2 text-sm font-medium text-ink-400 hover:text-ink-600">
            Clear
          </button>
        )}
      </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-2">
          <button
            ...
            className="rounded-md border border-ink-200 px-3 py-1.5 text-sm text-ink-500 disabled:opacity-40"
          >
            Previous
          </button>
