  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="font-mono text-xs text-ink-400">{ticket.ticketNumber}</p>
          <h2 className="mt-1 text-xl font-semibold text-ink-800">{ticket.subject}</h2>
        ...
        <div className="flex flex-wrap gap-2">
          {editAllowed && (
            <Link to={`/tickets/${ticket.id}/edit`} className="rounded-md border border-ink-200 px-3 py-1.5 text-sm font-medium text-ink-600 hover:bg-ink-50">
              Edit
            </Link>
          )}
          {assignAllowed && (
            <button onClick={() => setAssignOpen(true)} className="rounded-md border border-ink-200 px-3 py-1.5 text-sm font-medium text-ink-600 hover:bg-ink-50">
              {ticket.assignedAgent ? 'Reassign' : 'Assign'}
            </button>
          )}
          {deleteAllowed && (
            <button onClick={() => setDeleteOpen(true)} className="rounded-md border border-brick-200 px-3 py-1.5 text-sm font-medium text-brick-500 hover:bg-brick-50">
              Delete
            </button>
          )}
        </div>
      ...
      {statusOptions.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 rounded-lg border border-ink-100 bg-white p-3 shadow-card">
          <span className="text-xs font-medium uppercase tracking-wide text-ink-400">Move to:</span>
          {statusOptions.map((s) => (
            <button
              key={s}
              disabled={busyStatus === s}
              onClick={() => handleStatusChange(s)}
              className="rounded-md bg-ink-800 px-3 py-1.5 text-xs font-medium text-white hover:bg-ink-700 disabled:opacity-50"
            >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          <section className="rounded-lg border border-ink-100 bg-white p-5 shadow-card">
            <h3 className="mb-2 text-sm font-semibold text-ink-700">Description</h3>
            <p className="whitespace-pre-wrap text-sm leading-relaxed text-ink-600">{ticket.description}</p>
          </section>

          {ticket.resolution && (
            <section className="rounded-lg border border-moss-400/30 bg-moss-50 p-5">
              <h3 className="mb-2 text-sm font-semibold text-moss-600">Resolution</h3>
              <p className="whitespace-pre-wrap text-sm leading-relaxed text-ink-600">{ticket.resolution}</p>
              <p className="mt-2 text-xs text-ink-400">Resolved {formatDateTime(ticket.resolutionDate)}</p>
            </section>
          )}

          <section className="rounded-lg border border-ink-100 bg-white p-5 shadow-card">
            <h3 className="mb-3 text-sm font-semibold text-ink-700">Comments</h3>
        ...
        <div className="space-y-6">
          <section className="rounded-lg border border-ink-100 bg-white p-5 shadow-card">
            <h3 className="mb-3 text-sm font-semibold text-ink-700">Details</h3>
            <dl className="space-y-2.5 text-sm">
              <div className="flex justify-between gap-4">
                <dt className="text-ink-400">Created by</dt>
                <dd className="text-right text-ink-700">{ticket.createdByName}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-ink-400">Assigned agent</dt>
                <dd className="text-right text-ink-700">{ticket.assignedAgentName || 'Unassigned'}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-ink-400">Category</dt>
                <dd className="text-right text-ink-700">{ticket.category}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-ink-400">Preferred contact</dt>
                <dd className="text-right text-ink-700">{ticket.preferredContact}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-ink-400">Created</dt>
                <dd className="text-right text-ink-700">{formatDate(ticket.createdDate)}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-ink-400">Updated</dt>
                <dd className="text-right text-ink-700">{formatDate(ticket.updatedDate)}</dd>
              </div>
              {ticket.dueDate && (
                <div className="flex justify-between gap-4">
                  <dt className="text-ink-400">Due</dt>
                  <dd className="text-right text-ink-700">{formatDate(ticket.dueDate)}</dd>
                </div>
              )}
            </dl>
          </section>
          ...
          <section className="rounded-lg border border-ink-100 bg-white p-5 shadow-card">
            <h3 className="mb-3 text-sm font-semibold text-ink-700">Activity</h3>
      ...
      <Modal
        ...
        footer={
          <>
            <button onClick={() => setDeleteOpen(false)} className="rounded-md border border-ink-200 px-4 py-2 text-sm font-medium text-ink-600 hover:bg-ink-50">
              Cancel
            </button>
