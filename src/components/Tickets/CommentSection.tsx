    ...
    <div className="space-y-4">
      {comments.length === 0 ? (
        ...
        <ul className="space-y-3">
          {comments.map((c) => (
            <li key={c.id} className="rounded-md border border-ink-100 bg-ink-50/50 p-3">
              ...
              <p className="mt-1 text-sm text-ink-600">{c.comment}</p>
            </li>
          ))}
        </ul>
      )}
      ...
      {canComment && (
        <div className="flex gap-2 pt-1">
          <input
            ...
            className="flex-1 rounded-md border border-ink-200 px-3 py-2 text-sm focus:border-steel-400 focus:outline-none focus:ring-2 focus:ring-steel-100"
