...
export default function CategoryForm({ initial, onSubmit, onCancel }: CategoryFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="mb-1 block text-sm font-medium text-ink-600">Category name</label>
        <input value={name} onChange={(e) => setName(e.target.value)} className="w-full rounded-md border border-ink-200 px-3 py-2 text-sm focus:border-steel-400 focus:outline-none focus:ring-2 focus:ring-steel-100" />
        {errors.name && <p className="mt-1 text-xs text-brick-500">{errors.name}</p>}
      </div>
      <div>
        <label className="mb-1 block text-sm font-medium text-ink-600">Description</label>
        <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="w-full rounded-md border border-ink-200 px-3 py-2 text-sm focus:border-steel-400 focus:outline-none focus:ring-2 focus:ring-steel-100" />
      ...
      <div>
        ...
        <select value={status} onChange={(e) => setStatus(e.target.value as 'active' | 'inactive')} className="w-full rounded-md border border-ink-200 px-3 py-2 text-sm focus:border-steel-400 focus:outline-none focus:ring-2 focus:ring-steel-100">
